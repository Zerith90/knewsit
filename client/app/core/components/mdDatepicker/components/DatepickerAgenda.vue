    ...
    export default {
        ...
        watch: {
            ...
            yearsVisible(val, oldval) {
                ...
                let element = document.getElementsByClassName("datepicker-years")[0]
                this.scrollTo(element,scrollOffset,100)
            }
        },
        created() {
        ...
        methods: {
            dateFormatted() {
                ...
                if (this.dateObj.doubled) {
                    if(this.dateObj.startDate && this.dateObj.endDate){
                        return this.dateObj.startDate.format("DD MMM YYYY") + ' - ' + this.dateObj.endDate.format("DD MMM YYYY")
                    }
                    
                }
                if(this.dateObj.startDate) return this.dateObj.startDate.format("DD MMM YYYY");
                return moment().format("DD MMM YYYY");
            },
            scrollTo(element,to,duration){
                //method dont allow for recursion. this.scrollTo(x,x,x) doesnt work in the st timeout functon
            ...
            isDisabled(day) {
                if(this.disableFrom) return day.startOf('day').unix()<new moment(this.disableFrom).unix() 
                if (this.disablePassedDays) return day < moment().startOf('day');
                //if more than current day - disable
                if(moment().startOf('day').unix()< day.startOf('day').unix() && !this.disablePassedDays && !this.disableFrom) return true;

                return false;
            },
            isSelected(day) {
                if(this.dateObj.startDate && this.dateObj.endDate && this.dateObj.range){
                    let range = moment.range(this.dateObj.startDate,this.dateObj.endDate);
                    return day.within(range)
                }else if (this.dateObj.startDate){
                    return this.dateObj.startDate.unix() === day.unix();
                }else{
                    return moment().unix() === day.unix()
                }
            },
            selectDate(newDate) {
                this.classDirection = 'off';
                this.setClassDirection(newDate);
                if (!this.isDisabled(newDate) && this.dateObj.range) {
                   
                    // this.date = newDate.clone();
                    //check for start date
                    if(!this.dateObj.startDate){
                        this.dateObj.startDate={};
                        this.dateObj.startDate = newDate;

                    } else if(!this.dateObj.endDate){
                        this.cleanDate(this.dateObj.startDate,newDate);

                    }else{
                    //if new date is before start date or after end date, extend the date range
                        if( (newDate.isAfter(this.dateObj.startDate) && newDate.isBefore(this.dateObj.endDate))) {
                            if(!this.dateObj.prev){
                                this.cleanDate(newDate,this.dateObj.startDate)
                            }else{
                                this.cleanDate(newDate,this.dateObj.endDate)
                            }
                            this.dateObj.prev=!this.dateObj.prev
                        }else if( newDate.isAfter(this.dateObj.startDate) && newDate.isBefore(this.dateObj.endDate)){
                            if(!this.dateObj.prev){
                                this.cleanDate(newDate,this.dateObj.startDate)
                            }else{
                                this.cleanDate(newDate,this.dateObj.endDate)
                            ...
                            this.dateObj.prev=!this.dateObj.prev
                        }else if(newDate.isAfter(this.dateObj.endDate)){
                            this.cleanDate(newDate,this.dateObj.startDate);

                        }else if(newDate.isBefore(this.dateObj.startDate)){
                            this.cleanDate(newDate,this.dateObj.endDate)
                        }
                    }
                    
                }else{
                    this.dateObj.startDate = newDate.clone();
                }
                this.$emit('change', this.dateObj);
            },
            cleanDate(date1,date2){
                if(date1.isBefore(date2)){
                    this.dateObj.startDate=date1;
                    this.dateObj.endDate=date2;
                }else{
                    this.dateObj.startDate=date2;
                    this.dateObj.endDate=date1;
                }
            },
            selectYear(date) {
...
<style lang="sass">
    @import '../../../../../scss/themes/blurred/_variables';
    ...
    .datepicker {
        ...
        -webkit-box-shadow: 5px 22px 95px rgba(0,0,0,.24);
        -moz-box-shadow: 5px 22px 95px rgba(0,0,0,.24);
        box-shadow: 5px 22px 95px rgba(0,0,0,.24);
